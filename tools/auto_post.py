...
# 비용 줄이려면 사이즈를 낮추는 게 체감 큼
# 기존 1536x1024 -> 1024x1024로 기본 낮춤
...
def wikimedia_image_urls(query: str, limit: int = 12):
    """
...
def ensure_images_text_matched(slug: str, keyword: str, category: str, body_html_with_markers: str):
    ...
    - AI는 포스팅당 최대 1장
    - SVG placeholder 금지
    - 무료가 부족하면 검색어 확장으로 끝까지 무료를 먼저 시도
    - 마지막 1장만 AI 허용
    """
    ...
    for i in range(IMG_COUNT):
        ...
        for q in queries:
            try:
                urls = wikimedia_image_urls(q, limit=16)
                random.shuffle(urls)
                for u in urls:
        ...
        # 2) 무료가 다 실패했으면 AI는 "포스팅당 1장만" 허용
        if not got and IMAGE_PROVIDER == "openai" and not ai_used:
            generate_image_openai(ctx or "premium photorealistic blog photo", jpg_path)
            if not is_image_ok(jpg_path):
                raise RuntimeError(f"OpenAI image generation produced an invalid file: {slug}/{n}.jpg")
            ai_used = True
            got = True

        # 3) 여기까지 왔는데도 못 구하면 중단 (파란칸 방지)
        if not got:
            raise RuntimeError(f"Failed to fetch a free image and AI quota used. Post={slug} img={n}")
...
# -----------------------------
# Prompting (2-step to vary outline)
# -----------------------------
...
The outline must differ by topic and must not reuse a generic template.
...
Rules:
- 6 to 9 H2 sections.
- Each H2 has 1 to 3 H3 items.
- bullets are specific and topic-bound.
- FAQ has 4 to 6 Q&A.
- No markdown fences.
""".strip()
...
Use this outline JSON (follow it closely but write naturally):
{outline_json}
...
- Include these sections somewhere: Practical tips, Quick checklist
...
Length
- 3200 to 4200 words
- Use enough paragraphs. Avoid short sections.
